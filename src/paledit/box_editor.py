...
def apply_box_plan(world_path: Path, *, expected_sha256: str | None = None) -> dict[str, object]:
    ...
    if found != planned:
        raise ValueError(f"未完整定位 14 个箱子标签：{len(found)}/14")
...
def apply_box_plan_with_backup(world_path: Path, *, expected_sha256: str | None = None) -> dict[str, object]:
    world_path = world_path.expanduser().resolve()
    ...
    result = apply_box_plan(world_path, expected_sha256=expected_sha256)
