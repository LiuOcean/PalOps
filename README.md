...
服务器写入任务的安全快照保存在 `Saved/` 外的 `palops-backups/`，不会被游戏的定时归档再次打包。箱子任务只备份实际改写的 `Level.sav` 并记录 SHA-256 清单；默认保留 7 天且至少保留最近 3 份。游戏原生滚动备份、服务器定时归档与 PalOps 写入前快照是三套独立数据。
