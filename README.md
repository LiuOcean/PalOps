...
一个用来管理 Palworld 1.0 专用服务器、查看和修改存档的中文 Web 工具。
...
- 查看和修改常用服务器设置。
- 同步并解析 Palworld 1.0 存档，查看玩家、帕鲁、背包、储物箱、公会和据点。
- 搜索世界中的道具和帕鲁，修改玩家数据及背包物品。
- 管理本地备份，在地图上查看玩家、据点、传送点和塔。
- 浏览中文道具、帕鲁和被动技能图鉴。
...
查看玩家、背包、帕鲁、储物箱、公会与备份；存档写入前会自动备份并验证结果。
...
PalOps 会动态发现世界目录。从服务器同步时，存档会先进入临时目录并通过解析检查，再替换本地 `Save/`；旧版本保存在 `.paledit-backups/`。网页中的存档修改只作用于本地副本，不会在服务器运行时直接替换远端存档。

每次写入都会检查文件变更、创建备份、写入临时文件并重新解析；任何一步失败都不会替换原文件。Palworld 更新或解析依赖变化后，请先用备份副本验证兼容性。
...
主要代码位于 `src/paledit/`，图鉴更新脚本位于 `tools/`。涉及存档写入或服务器操作的改动，请同时说明备份、恢复方式和验证结果。
